...
class RoutingTable(object):
    ...
    def touchKBucket(self, key):
        ...
        @type key: str
        """
    def optimized_removeContact(self, contactID):
        """
        Equal at removeContact but delete immediatly the contact
        """
class TreeRoutingTable(RoutingTable):
    ...
    def optimized_removeContact(self, contactID):
        ...
        @type contactID: str
        """
        print "inside optimized_removeContact Andrea Modification"
        bucketIndex = self._kbucketIndex(contactID)
        try:
            #andrea
            #print "Andrea Modification"
            index=self._buckets[bucketIndex]._contacts.index(contactID)
            contact = self._buckets[bucketIndex]._contacts.pop(index)
            self._buckets[bucketIndex]._contacts(contact)
        except ValueError:
            #print 'removeContact(): Contact not in routing table'
            return
        #contact.failedRPCs += 1
        #if contact.failedRPCs >= 2:        
        #   self._buckets[bucketIndex].removeContact(contactID)
...
def optimized_removeContact(self, contactID):
        ...
        try:
            #andrea
            print "Andrea Modification"
            index=self._buckets[bucketIndex].index(contactID)
            contact = self._buckets[bucketIndex]._contacts.pop(index)
            self._buckets[bucketIndex]._contacts(contact)
        except ValueError:
            print 'removeContact(): Contact not in routing table'
